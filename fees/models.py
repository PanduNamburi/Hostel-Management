...
class FeeRecord(models.Model):
    ...
    def amount_paid(self):
        return sum(payment.amount for payment in self.payments.filter(status='approved'))

    def fee_due(self):
        from decimal import Decimal
        return max(Decimal('0.00'), self.fee_amount - self.amount_paid())

    def is_fully_paid(self):
        return self.fee_due() <= 0
